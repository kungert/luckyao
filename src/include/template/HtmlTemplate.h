...
namespace luckyao
{
    ...
    public:
         ...
         * @return
         */
        HtmlTemplate &setValue(const Json &obj)
        {
            // var_tables = std::move(const_cast<Json &>(obj).getData());
            m_tables = std::move(obj);
            return *this;
        }
    ...
    private:
         ...
         */
        double calculator(vector<Json> &number, vector<char> &op);
         ...
         * @return
         */
        Json parse_var(const string &tmp_str);
    ...
    private:
        string tmpl;
        // map<string, Json> var_tables; // 变量映射表
        Json m_tables;
        FILE *file = nullptr;
